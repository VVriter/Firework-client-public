...
@ModuleManifest(
        ...
        description = "Draws trajectories"
)
public class Trajectories extends Module {
   @Subscribe
   public Listener<Render3dE> listener = new Listener<>(e-> {
       if (mc.world != null && mc.player != null && mc.getRenderManager() != null) {
           final double renderPosX = mc.player.lastTickPosX + (mc.player.posX - mc.player.lastTickPosX) * 1;
           final double renderPosY = mc.player.lastTickPosY + (mc.player.posY - mc.player.lastTickPosY) * 1;
           final double renderPosZ = mc.player.lastTickPosZ + (mc.player.posZ - mc.player.lastTickPosZ) * 1;
           ...
           if (mc.gameSettings.thirdPersonView == 0 && (mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemBow || mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemFishingRod || mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemEnderPearl || mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemEgg || mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemSnowball || mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemExpBottle)) {
               ...
               final int var6 = 72000 - mc.player.getItemInUseCount();
               ...
               motionY /= distance;
               motionZ /= distance;
               final float pow = (item instanceof ItemBow) ? (power * 2.0f) : ((item instanceof ItemFishingRod) ? 1.25f : ((mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() == Items.EXPERIENCE_BOTTLE) ? 0.9f : 1.0f));
               ...
               while (!hasLanded && posY > 0.0) {
                   ...
                   final RayTraceResult possibleLandingStrip = mc.world.rayTraceBlocks(present, future, false, true, false);
