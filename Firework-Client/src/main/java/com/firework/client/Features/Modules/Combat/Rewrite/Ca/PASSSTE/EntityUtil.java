...
public class EntityUtil
    ...
    public static boolean isPlayerValid(final EntityPlayer player, final float range) {
        return player != mc.player && mc.player.getDistance(player) < range && !isDead(player);
    }
    ...
    public static boolean checkForLiquid(Entity entity, boolean b) {
        ...
        for (int i = MathHelper.floor((double)entity.posX); i < MathHelper.ceil((double)entity.posX); ++i) {
            for (int j = MathHelper.floor((double)entity.posZ); j < MathHelper.ceil((double)entity.posZ); ++j) {
                if (!(mc.world.getBlockState(new BlockPos(i, MathHelper.floor((double)n2), j)).getBlock() instanceof BlockLiquid)) continue;
                return true;
            }
        }
        return false;
    ...
    public static boolean isAboveLiquid(Entity entity) {
        ...
        for (int i = MathHelper.floor((double)entity.posX); i < MathHelper.ceil((double)entity.posX); ++i) {
            for (int j = MathHelper.floor((double)entity.posZ); j < MathHelper.ceil((double)entity.posZ); ++j) {
                if (!(mc.world.getBlockState(new BlockPos(i, (int)n, j)).getBlock() instanceof BlockLiquid)) continue;
                return true;
            }
        }
        return false;
    ...
    public static boolean isOnLiquid(double offset) {
        ...
        AxisAlignedBB bb = mc.player.getRidingEntity() != null ? mc.player.getRidingEntity().getEntityBoundingBox().contract(0.0, 0.0, 0.0).offset(0.0, -offset, 0.0) : mc.player.getEntityBoundingBox().contract(0.0, 0.0, 0.0).offset(0.0, -offset, 0.0);
    ...
    public static boolean isOnLiquid() {
        ...
        for (int x = MathHelper.floor((double)mc.player.posX); x < MathHelper.ceil((double)mc.player.posX); ++x) {
            for (int z = MathHelper.floor((double)mc.player.posZ); z < MathHelper.ceil((double)mc.player.posZ); ++z) {
    ...
    public static Vec3d getInterpolatedRenderPos(Vec3d vec) {
        return new Vec3d(vec.x, vec.y, vec.z).subtract(mc.getRenderManager().renderViewEntity.posX, mc.getRenderManager().renderViewEntity.posY, mc.getRenderManager().renderViewEntity.posZ);
    }
