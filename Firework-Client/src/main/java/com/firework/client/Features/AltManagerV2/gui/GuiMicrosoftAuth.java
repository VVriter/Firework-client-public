...
import com.firework.client.Features.AltManagerV2.auth.MicrosoftAuth;
...
public class GuiMicrosoftAuth extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.add(cancelButton = new GuiButton(
                0, width / 2 - 100, height / 2 + fontRenderer.FONT_HEIGHT / 2 + fontRenderer.FONT_HEIGHT, "Cancel"
        ));
        if (task == null) {
            if (executor == null) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(
                fontRenderer, "Microsoft Authentication",
                width / 2, height / 2 - fontRenderer.FONT_HEIGHT / 2 - fontRenderer.FONT_HEIGHT * 2, 11184810
        );
        if (status != null) {
            drawCenteredString(
                    fontRenderer, TextFormatting.translate(status),
                    width / 2, height / 2 - fontRenderer.FONT_HEIGHT / 2, -1
            );
        }
        if (cause != null) {
            final String causeText = TextFormatting.translate(cause);
            Gui.drawRect(
                    0, height - 2 - fontRenderer.FONT_HEIGHT - 2,
                    2 + mc.fontRenderer.getStringWidth(causeText) + 2, height,
                    0x64000000
            );
            Gui.drawRect(
                    0, height - 1,
            ...
            drawString(
                    fontRenderer, TextFormatting.translate(cause),
                    2, height - 2 - fontRenderer.FONT_HEIGHT, -1
            );
        }
    }
